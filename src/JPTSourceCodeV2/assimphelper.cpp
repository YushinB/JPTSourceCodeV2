...
CModel::CModel(std::string const & path, bool gamma)
...
void CModel::GetRange(double * dRange)
{
	std::vector<double> vdX, vdY, vdZ;
	for (const auto& eachmesh : meshes)
	{/*No modify v vector*/
		for (const auto& eachver : eachmesh.vertices)
		{
			vdX.push_back(eachver.Position.x);
			vdY.push_back(eachver.Position.y);
			vdZ.push_back(eachver.Position.z);
		}
	}
